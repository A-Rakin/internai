...
### Step 4: Delete the Locked/Inconsistent Database
Since the database schema changed to use the custom user role system, you need a clean database. Run this command to delete the current database file:
```powershell
Remove-Item db.sqlite3 -Force
```
*(If you get a permission or file-lock error, make sure any running `runserver` processes are completely stopped!)*
